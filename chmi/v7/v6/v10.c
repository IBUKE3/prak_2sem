...
void cubic_spline (ld (*f)(ld x), int m, char* splinef) {
  ...
  ld *alphas = malloc(n*sizeof(ld));
  ld *betas = malloc(n*sizeof(ld));

  ld *c_array = malloc(n*sizeof(ld));
  ld *b_array = malloc(n*sizeof(ld));
  ld *d_array = malloc(n*sizeof(ld));
  ld *F_array = malloc(n*sizeof(ld));

  // в этих двух массивах нумерация 0..n
  ld *x_array = malloc(n*sizeof(ld));
  ld *y_array = malloc(n*sizeof(ld));
